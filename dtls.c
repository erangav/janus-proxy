...
static void janus_dtls_srtp_free(const janus_refcount *dtls_ref) {
	...
	if(dtls->local_policy.key) {
	    free(dtls->local_policy.key);
	    dtls->local_policy.key = NULL;
	}
    if(dtls->remote_policy.key) {
        free(dtls->remote_policy.key);
		...
		if(!rcert) {
			...
			*(rfp-1) = 0;
			...
			if(dtls->dtls_state == JANUS_DTLS_STATE_CONNECTED) {
				...
				dtls->remote_policy.key = (unsigned char *)malloc(master_length);
				...
				dtls->local_policy.key = (unsigned char *)malloc(master_length);
