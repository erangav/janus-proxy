...
//				}
				...
				janus_plugin_rtcp rtcp = { .buffer = buf, .length = buflen };
				janus_plugin *plugin = (janus_plugin *)handle->app;
...
//				}
				...
				if(res != srtp_err_status_ok) {
                    ...
                    guint16 seq = ntohs(header->seq_number);
                    JANUS_LOG(LOG_WARN, "[%"SCNu64"] restart SRTP because of protection error %s (ssrc=%u len=%d-->%d, ts=%"SCNu32", seq=%"SCNu16")...\n", handle->handle_id, janus_srtp_error_str(res), ssrc, pkt->length, protected, timestamp, seq);

					if(component->dtls->srtp_out) {
					    srtp_dealloc(component->dtls->srtp_out);
					    component->dtls->srtp_out = NULL;
					    srtp_err_status_t res_c = srtp_create(&(component->dtls->srtp_out), &(component->dtls->local_policy));
					    if(res_c != srtp_err_status_ok) {
					        /* Something went wrong... */
					        JANUS_LOG(LOG_ERR, "[%"SCNu64"] Oops, error creating outbound SRTP session for component %d in stream %d??\n", handle->handle_id, component->component_id, stream->stream_id);
					        JANUS_LOG(LOG_ERR, "[%"SCNu64"]  -- %d (%s)\n", handle->handle_id, res_c, janus_srtp_error_str(res_c));
					    } else {
			                res = janus_is_webrtc_encryption_enabled() ?
			                    srtp_protect(component->dtls->srtp_out, pkt->data, &protected) : srtp_err_status_ok;
					    }
					}
				}
			    if(res != srtp_err_status_ok) {
			        ...
			        guint32 ssrc = ntohl(header->ssrc);
			        guint16 seq = ntohs(header->seq_number);
			        JANUS_LOG(LOG_ERR, "[%"SCNu64"] dropping packet protection error %s (ssrc=%u len=%d-->%d, ts=%"SCNu32", seq=%"SCNu16")...\n", handle->handle_id, janus_srtp_error_str(res), ssrc, pkt->length, protected, timestamp, seq);
			    } else {
					/* Shoot! */
					int sent = nice_agent_send(handle->agent, stream->stream_id, component->component_id, protected, pkt->data);
					if(sent < protected) {
