...
//						if(strlen(a->value) > 16) { // omer cancel the restriction, no problem to have long mid as long as there in no mid extension.
...
#ifdef HAVE_SCTP
	*data = (strstr(jsep_sdp, "m=application") && !strstr(jsep_sdp, "m=application 0")) ? 1 : 0;	/* FIXME This is a really hacky way of checking... */
#else
	*data = 0;
#endif
...
//					|| !strcasecmp(a->name, "max-message-size")
		...
		tempA = m->attributes;
		while(tempA) { // omer: as long as junos do encryption it is safer to have this here.
			janus_sdp_attribute *a = (janus_sdp_attribute *)tempA->data;
...
char *janus_sdp_merge(void *ice_handle, janus_sdp *anon, gboolean offer) {
	...
	/* Session name s= */
	if(anon->s_name == NULL)
		anon->s_name = g_strdup("Eim Tirzo");
	...
	GList *temp = anon->attributes;
	int has_group_attr = 0;
	janus_sdp_attribute *a=NULL;
	while(temp) {
       janus_sdp_attribute *g = (janus_sdp_attribute *)temp->data;
       if(!strcasecmp(g->name, "group")) {
               has_group_attr = 1;
               a=g;
               JANUS_LOG(LOG_INFO, "[%"SCNu64"] group attribute found: '%s'\n",
                               handle->handle_id, g->value);
               break;
        }
   ...
   if (0 == has_group_attr)  {
           ...
           JANUS_LOG(LOG_WARN, "[%"SCNu64"] No group attribute was found, new one will be created.\n",
                           handle->handle_id);
           ...
           while(temp) {
                   ...
                   if(mid) {
                           g_snprintf(buffer_part, sizeof(buffer_part), " %s", mid);
                   }
                   else if (m->port != 0 ) {
                           g_snprintf(buffer_part, sizeof(buffer_part), " m%d", m_line_count);
                           JANUS_LOG(LOG_WARN, "[%"SCNu64"] No mid= attribute for m-line %d ...\n",
                                           handle->handle_id, m_line_count);
                   }
                   if (m->port != 0 ) {
   ...
   //#endif
           ...
           a = janus_sdp_attribute_create("group", "%s", buffer);
