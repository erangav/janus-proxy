...
int janus_sdp_get_codec_pt(janus_sdp *sdp, const char *codec) {
	...
	if(!strcasecmp(codec, "opus")) {
		...
		format = "amr/8000";
		format2 = "AMR/8000";
	} else if(!strcasecmp(codec, "amr-wb")) {
		format = "amr-wb/16000";
		format2 = "AMR-WB/16000";
			...
			if(a->name != NULL && a->value != NULL && !strcasecmp(a->name, "rtpmap")) {
				...
				if(a_pt == pt) {
					...
					if(strstr(a->value, "g722") || strstr(a->value, "G722"))
						return "g722";
					if(strstr(a->value, "amr") || strstr(a->value, "AMR"))
						return "amr";
					if(strstr(a->value, "amr-wb") || strstr(a->value, "AMR-WB"))
						return "amr-wb";
...
const char *janus_sdp_get_codec_rtpmap(const char *codec) {
	...
	if(!strcasecmp(codec, "isac32"))
		return "ISAC/32000";
	if(!strcasecmp(codec, "amr"))
		return "AMR/8000";
	if(!strcasecmp(codec, "amr-wb"))
		return "AMR-WB/16000";
