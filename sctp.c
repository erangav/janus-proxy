...
static const char *default_label = "mav-d";
...
void janus_sctp_data_from_dtls(janus_sctp_association *sctp, char *buf, int len) {
	if(sctp == NULL || sctp->handle == NULL || buf == NULL || len <= 0)
		return;
	JANUS_LOG(LOG_INFO, "[%"SCNu64"] Data from DTLS to SCTP stack: %d bytes\n", sctp->handle_id, len);
#ifdef DEBUG_SCTP
	if(sctp->debug_dump != NULL) {
...
//	usrsctp_conninput((void *)sctp, buf, len, 0);
	janus_dtls_notify_data(sctp->dtls, NULL, 0, buf, len);
}
...
int janus_sctp_data_to_dtls(void *instance, void *buffer, size_t length, uint8_t tos, uint8_t set_df) {
	...
	if(sctp == NULL || sctp->handle == NULL)
		return -1;
	JANUS_LOG(LOG_INFO, "[%"SCNu64"] Data from SCTP to DTLS stack: %zu bytes\n", sctp->handle_id, length);
#ifdef DEBUG_SCTP
	if(sctp->debug_dump != NULL) {
...
void janus_sctp_send_data(janus_sctp_association *sctp, char *label, gboolean textdata, char *buf, int len) {
	...
	JANUS_LOG(LOG_VERB, "[%"SCNu64"] SCTP data to send (%d bytes) coming from a plugin.\n",
		  sctp->handle_id, len);
	//SERG send from plugin
//	janus_sctp_data_to_dtls(void *instance, void *buffer, size_t length, uint8_t tos, uint8_t set_df) {
//		janus_sctp_association *sctp = (janus_sctp_association *)instance;
	if(0 != janus_sctp_data_to_dtls((void*)sctp, (void*)buf, len, 0, 0) ) {
		JANUS_LOG(LOG_ERR, "[%"SCNu64"] SCTP data FAILED to send (%d bytes) coming from a plugin\n",
				sctp->handle_id, len);
	}
